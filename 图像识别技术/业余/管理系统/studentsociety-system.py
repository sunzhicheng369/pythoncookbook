...
def insert():
    ...
    while mark:
        ...
        try:
            sex = input("请输入该同学的性别：")
            age = int(input("请输入该同学的年龄："))
            zhi = input("请输入该同学的职务：")
            bumen = input("请输入该同学的部门：")
            tell = int(input("请输入该同学的电话号码："))
        except:
            print("输入时存在错误，输入无效．重新录入信息")
            continue
        stdent = {"id": id, "name": name, "sex": sex, "age": age, "zhi": zhi,"bumen":bumen,"tell":tell}  # 将输入的学生信息保存到字典
...
def show_student(studentList):
    if not studentList:
        print("无数据信息查询\n")
        return
    format_title = "{:^6}{:^12}\t{:^6}\t{:^6}\t{:^10}\t{:^10}\t{:^12}"
    print(format_title.format("ID", "名 字", "性 别", "年 龄", "职 业", "部 门", "电 话"))
    format_data = "{:^6}{:^12}\t{:^8}\t{:^10}\t{:^12}\t{:^12}\t{:^12}"
    for info in studentList:
        print(format_data.format(info.get("id"), info.get("name"), str(info.get("sex")), str(info.get("age")),
                                 str(info.get("zhi")),str(info.get("bumen")),str(info.get("tell"))))
